...
class Zoo_visitor
{
		...
		if(isset($this->settings['member_channel_id'])){
			...
			if(isset($this->settings['member_channel_name']) && $this->settings['member_channel_name'] != ''){
				...
				if($query_cmg->num_rows() == 0){
					...
					$cmg = array('channel_id' => $this->settings['member_channel_id'], 'group_id' => $this->EE->session->userdata['group_id']);
	...
	function get_params(){
		...
		// if(!empty($this->EE->TMPL->tagparams))
		// 	{
			foreach ($this->EE->TMPL->tagparams as $key => $value)
			{
	...
	function forgot_password(){
		...
		return $this->EE->zoo_visitor_lib->forgot_password($this->EE->TMPL->tagdata, $return, $error_handling, $is_ajax_request);
	}
	...
	function members(){
		...
		$this->EE->TMPL->tagparams['require_entry'] = (isset($entry_id)) ? 'yes' : 'no';
