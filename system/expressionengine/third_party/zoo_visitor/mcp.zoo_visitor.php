...
class Zoo_visitor_mcp
{
	...
	function sync()
	{
		...
		// ==========================
		$vars['standard_member_fields'] = array('url', 'location', 'occupation', 'interests', 'aol_im', 'yahoo_im', 'msn_im', 'icq', 'bio', 'bday_y', 'bday_m', 'bday_d','signature');//, 'avatar_filename', 'photo_filename');
	...
	function settings_form(){
		...
		// Load view
		return $this->_content_wrapper('settings',  $this->module_name." Settings", $vars);
	...
	function install_visitor(){
		...
		if ($query->num_rows() == 0){
			...
			$data['password']	= (version_compare(APP_VER, '2.4.0', '<')) ? $this->EE->functions->hash('zoo_visitor_login') : do_hash('zoo_visitor_login');
		...
		// = Create templates =
		// ====================
		$templates = array('index','menu','register','profile','login', 'login_ajax', 'change_password','change_login','forgot_password');
	...
	function create_field($channel_id, $group_id, $mfield)
	{
		...
		if(isset($mfield->m_field_maxl)){ $native_settings['field_maxl'] = $mfield->m_field_maxl; }
