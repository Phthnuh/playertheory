...
*/
		...
		$this->VAL = new EE_Validate(
								array(
										...
										'fetch_lang'		=> FALSE,
										'require_cpw'		=> ($this->EE->cp->allowed_group('can_admin_members')) ? FALSE : TRUE,
										'enable_log'		=> TRUE,
										'username'			=> $_POST['username'],
		...
		{
			return array('result'=>'failed', 'errors' => $this->VAL->errors);
		}
		...
		if($save){
			...
			// Write log file
			$this->EE->logger->log_action($this->VAL->log_msg);
	
	/*
			$message = $this->EE->lang->line('settings_updated');
...
*/
		...
		$this->VAL = new EE_Validate(
								array(
										...
										'val_type'			=> 'update', // new or update
										'fetch_lang'		=> FALSE,
										'require_cpw'		=> ($current_email == $_POST['email'] || $this->EE->cp->allowed_group('can_admin_members')) ? FALSE : TRUE,
										'enable_log'		=> TRUE,
										'email'				=> $_POST['email'],
		...
		$query_statusgroups = $this->EE->db->select('group_id')->where('group_name','Zoo Visitor Membergroup')->get('status_groups');//->where('site_id',$this->EE->config->item('site_id'))->get('status_groups');
	 	...
	 	if ($query_statusgroups->num_rows() > 0){
			...
			if ($query_statusses->num_rows() == 0){
				...
				$this->EE->db->insert('statuses', array('status' => $status, 'group_id' => $query_statusgroups->row()->group_id, 'site_id' => $this->EE->config->item('site_id')) );
			}
		}	
	}
