...
class Zoo_visitor_lib
{
	...
	function forgot_password($tagdata, $ret, $error_handling, $is_ajax_request = 'no')
	{
		...
		$id = $this->EE->TMPL->fetch_param('id', $this->EE->TMPL->fetch_param('form_id', ''));
		$id = ($id == '') ? '' : ' id="' . $id . '"';

		$class = $this->EE->TMPL->fetch_param('class', $this->EE->TMPL->fetch_param('form_class', ''));
		$class = ($class == '') ? '' : ' class="' . $class . '"';

		$action = ($action != '') ? ' action="' . $action . '"' : '';

		$form_declared = '<form id="forgot_password_form" method="post" ' . $action . ' ' . $id . ' ' . $class . '>
	...
	function _retrieve_password()
	{
		...
		if (!$this->EE->email->send()) {
			return array('error' => lang('error_sending_email'));
		}
		...
		if (array_key_exists('password', $_POST) || array_key_exists('username', $_POST) || array_key_exists('screen_name', $_POST)) {
			...
			if ((isset($_POST['username']) && $_POST['username'] != $this->EE->session->userdata['username']) ||
				...
				if (!isset($_POST['username'])) {
					$_POST['username'] = $this->EE->session->userdata['username'];
				}
	...
	function update_native_member_fields($member_id = '', $entry_id = NULL)
	{
		...
		$data   = array();
		$fields = array('bday_y', 'bday_m', 'bday_d', 'birthday', 'url', 'location', 'occupation', 'interests', 'aol_im', 'icq', 'yahoo_im', 'msn_im', 'bio', 'signature', 'avatar', 'photo', 'timezones', 'daylight_savings', 'time_format', 'language');
	...
	function update_email($do_update = TRUE, $member_id)
	{
		...
		$VAL = new EE_Validate(
			array(
				...
				'fetch_lang'       => TRUE,
				'require_cpw'      => TRUE, //ADDED IN ZOO VISITOR
				'enable_log'       => FALSE,
				'email'            => $_POST['email'],
	...
	function register_member($ext, $doRegister = TRUE, $error_handling = '')
	{
		...
		if ($this->EE->config->item('use_membership_captcha') == 'y') {
			...
			if ($query->row('count') == 0) {
				return array('submission', array($this->EE->lang->line('captcha_incorrect')));
			}
